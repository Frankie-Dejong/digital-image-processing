...
void help() {
    ...
    std::cout << "-fft, --FFT: Apply FFT transformation to the image" << std::endl;
    std::cout << "-cos, --cosine: Apply cosine transformation to the image" << std::endl;
}
...
ArgParser::ArgParser(int argc, char** argv) {
    ...
    for (int i = 1;i < argc;) {
        ...
        else if (!strcmp(argv[i], "-fft") || !strcmp(argv[i], "--FFT")) {
            need_fft_transform = true;
        }
        else if (!strcmp(argv[i], "-cos") || !strcmp(argv[i], "--cosine")) {
            need_cos_transform = true;
        }
        else {
...
bool ArgParser::needFFTTransform() const {
    return need_fft_transform;
}

bool ArgParser::needCosTransform() const {
    return need_cos_transform;
}